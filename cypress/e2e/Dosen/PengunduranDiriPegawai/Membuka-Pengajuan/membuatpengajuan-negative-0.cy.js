...
describe("Membuat Pengajuan Pengunduran Diri Pegawai - Negative Test (File Surat Pengunduran Diri Bermaterai Kosong)", () => {
    beforeEach(() => {
        ...
        // Klik dropdown "Rekrutmen" di sidebar
    ...
    it("Gagal submit dengan file Surat Pengunduran Diri Bermaterai kosong", () => {
        // Verifikasi form pengajuan muncul
        cy.contains("Form Pengajuan Pengurusan Pengunduran Diri Pegawai").should("be.visible");

        // Tunggu form sepenuhnya dimuat
        cy.wait(1000);

        // Pastikan input file kosong (tidak upload)
    ...
    cy.log("⚠️ Surat Pengunduran Diri Bermaterai tidak diupload");
    ...
    // Pastikan tetap di halaman yang sama (submit gagal)
    cy.url().should(
        "include",
        "/dosen/kepegawaian/rekrutmen/pengunduran-diri/pegawai-pns/create"
    );

    cy.log("✅ Validasi berhasil: Submit gagal karena file upload kosong");
    });
});
