...
describe("Membuat Pengajuan Surat Keterangan Kerja - Negative Test (Semua Field Kosong)", () => {
  beforeEach(() => {
    ...
    // Klik menu "Pengajuan Surat Keterangan Kerja"
    cy.contains("Pengajuan Surat Keterangan Kerja").click();

    // Verifikasi berhasil membuka halaman pengajuan
    cy.url().should(
      "include",
      "/dosen/kepegawaian/umum/pengajuan-keterangan-kerja/create"
    );
  });

  it("Tidak boleh submit dengan semua field kosong", () => {
    ...
    // SEMUA FIELD TIDAK DIISI (dikosongkan)

    // Tanggal Mulai Kerja - kosongkan
    cy.get('input[name="tgl_mulai"]').clear({ force: true });
    cy.log("⚠️ Tanggal Mulai Kerja dikosongkan");

    // Verifikasi field tanggal mulai kosong
    cy.get('input[name="tgl_mulai"]').should("have.value", "");

    // Tanggal Selesai Kerja - kosongkan
    cy.get('input[name="tgl_selesai"]').clear({ force: true });
    cy.log("⚠️ Tanggal Selesai Kerja dikosongkan");

    // Verifikasi field tanggal selesai kosong
    cy.get('input[name="tgl_selesai"]').should("have.value", "");

    // Keperluan Surat Keterangan Kerja - kosongkan
    cy.contains("Keperluan Surat Keterangan Kerja")
      .parent()
      .find("input")
      .clear();

    cy.log("⚠️ Keperluan Surat Keterangan Kerja dikosongkan");

    // Verifikasi field keperluan kosong
    cy.contains("Keperluan Surat Keterangan Kerja")
      .parent()
      .find("input")
      .should("have.value", "");
    ...
    cy.log("✅ Validasi berhasil: Submit gagal karena semua field kosong");
  });
});
