...
describe("Melihat Profil Dosen", () => {
  beforeEach(() => {
    // Login terlebih dahulu
    loginDosen("andika.setiawan@if.itera.ac.id", "SS04ndika");
  });

  it("Harus berhasil melihat halaman profil dosen", () => {
    ...
    // Klik "Lihat Profil" pada kanan atas
    // Ini adalah div yang bisa diklik
    cy.contains("div", "Lihat Profil").click();

    // Tunggu halaman profil dimuat
    cy.wait(2000);

    // Verifikasi halaman profil muncul
    // Gunakan teks yang lebih umum atau URL untuk verifikasi
    cy.url().should("include", "/profi-dosen");

    // Verifikasi section Identitas Dosen muncul
    cy.contains("Detail Data dan File Umum Dosen").should("be.visible");

    cy.log("✅ Halaman profil dosen berhasil ditampilkan!");
  });
});
